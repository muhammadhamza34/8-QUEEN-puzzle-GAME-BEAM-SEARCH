 ...
 */
package com.mycompany.beamqueen;
...
public class BoardState implements Comparable<BoardState>
{
    ...
    {
        ArrayList<BoardState> children = new ArrayList<BoardState>();//child ki current state
        ...
        for(int i=0; i<N; i++)//collumns
        {
            for(int j=1; j<N; j++)//moves
            {
                if(this.placements[i]+j<=N)
                {
                    ...
                    for (int k=0; k<N; k++){newPlacementsDown[k] = this.placements[k];}//current state ki  placemnts copy kry ga 
